...
//! # Colors
//!
//! Pico Logger optionally supports colors if the `color` feature is enabled and if the environment supports colors
//! (see [the `owo-colors` crate documentation](https://docs.rs/owo-colors/latest/owo_colors/index.html#only-style-on-supported-terminals)).
    ...
    fn log(&self, record: &Record) {
        if self.enabled(record.metadata()) {
            ...
            #[cfg(feature = "color")]
            {
                eprintln!(
                    "{} {}",
                    label.if_supports_color(Stream::Stderr, |s| s.color(level_color(level))),
                    record.args()
                );
            }
        }
